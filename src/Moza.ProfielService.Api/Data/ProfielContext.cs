...
namespace Moza.ProfielService.Api.Data;
...
public class ProfielDbContext(DbContextOptions<ProfielDbContext> options, IHttpContextAccessor httpContextAccessor) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<EmailEntity>()
            .HasQueryFilter(e => !e.Onderneming.IsDeleted)
    ...
    public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        var addedEntries = this.ChangeTracker.Entries<OndernemingEntity>()
            .Where(e => e.State == EntityState.Added)
            .ToList();

        var logs = this.ChangeTracker.Entries<OndernemingEntity>()
            ...
            .SelectMany(e =>
            {
                return e.State switch
                {
                    ...
                    [
                        ...
                        {
                            KvkNummer = e.Entity.KvkNummer, Action = "DELETE", PerformedBy = user, PerformedAt = now
                        }
                    ],
                    _ => []
                };
        ...
        if (logs.Count != 0)
        {
            this.OndernemingenAuditLog.AddRange(logs);
